...
Este sistema cuenta con un receptor y un emisor, separados lo suficiente para que un vehiculo pase entre ellos. La captura se da cuando el vehiculo interrumpe el paso de luz entre el receptor y el emisor.

El sistema de barrera infrarroja cuenta con una serie de inconvenientes que dificultan su implementación como:

\begin{enumerate}
    \item Potencia minima para activar el receptor.
...
Para la lectura de distancia se implementó un librería llamada \textit{us100}, la cual utiliza \textit{pyserial} [ref a pyserial] para la comunicación por puerto serie. Esta librería posee una función la cual devuelve la distancia de un objetivo, pero con la finalidad de disminuir un objeto que se cruza o una mala medición se utiliza una lista anidada que posee las últimas 5 mediciones, y devuelve el promedio de las mismas.
...
Para la conexión con el servidor MQTT se utilizó \textit{paho-mqtt} [ref paho], escuchando el tópico \textit{$id/config$} donde el $id$ es un identificador único por cada placa.

En cuanto al envío de los registros este se realiza por POST mediante HTTP utilizando la librería \textit{requests} [ref requers].
...
El algoritmo implementado en el sistema SL mini se puede apreciar en la Fig. \ref{fig:algoritmo-sl-mini}. Debido a la implementación de las librerías auxiliares el código principal del SL mini es sencillo.
\begin{figure}
    \centering

    \caption{Algoritmo implementado en el sistema SL mini.}
    \label{fig:algoritmo-sl-mini}
\end{figure}

En cuanto al sistema SL este cuenta con un algoritmo un poco más complejo cuyo diagrama de flujo puede observarse en la Fig. \ref{fig:algoritmo-sl}. En este caso se añadio el algoritmo de OCR, para ello se compilo la YOLOV4 para sistemas ARM, y se compilaron los modelos de la CNN-OCR a la versión de tensorflow lite, ya que es la versión compatible con arquitectura ARM.

\begin{figure}
    \centering

    \caption{Algoritmo implementado en el sistema SL.}
    \label{fig:algoritmo-sl}
\end{figure}

Se puede observar que en ambos casos los drivers no difieren en demasiado, más allá de la implementación del algoritmo de OCR, es por ello que la tarea de implementar librerías facilita el proceso de actualización de drivers para las placas, e incluso aporta versatilidad a la hora de cambiar a una placa con un hardware diferente, siempre que este soporte Python 3.